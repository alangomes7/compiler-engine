...
public class ParserTableManager {
    ...
    /**
     * Populates the table with the given parse table data.
     * Dynamically adds a column for each terminal symbol.
     */
    public void populate(ParseTable parseTable) {
        ...
        Set<Symbol> terminals = innerTable.values().stream()
                .flatMap(row -> row.keySet().stream())
                .collect(Collectors.toSet());

        List<Symbol> sortedTerminals = new ArrayList<>(terminals);
        sortedTerminals.sort((t1, t2) -> {
        ...
        for (Symbol terminal : sortedTerminals) {
            ...
            terminalCol.setCellValueFactory(data -> {
                ...
                String text = prods.stream().map(Production::toString).collect(Collectors.joining("\n"));
