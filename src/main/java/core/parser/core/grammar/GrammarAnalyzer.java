...
public class GrammarAnalyzer {
    ...
    public static GrammarErrors analyzeGrammar(Grammar grammar) {
        return new GrammarErrors(
            getLeftRecursionDetails(grammar),
            getCommonPrefixDetails(grammar)
        );
    }
        ...
        for (Production p : grammar.getProductions()) {
            if (!p.getRhs().isEmpty()) {
                ...
                if (p.getLhs().equals(firstSymbol)) {
                    details.add(String.format("Direct Left-Recursion on Non-Terminal '%s': [%s]", 
                ...
                for (int j = i + 1; j < productions.size(); j++) {
                    ...
                    if (!rhs1.isEmpty() && !rhs2.isEmpty() && !rhs1.get(0).equals(Symbol.EPSILON)) {
                        if (rhs1.get(0).equals(rhs2.get(0))) {
                            details.add(String.format("Common Prefix '%s' on Non-Terminal '%s' between: [%s] and [%s]", 
                                    rhs1.get(0).getName(), 
                                    nonTerminal.getName(), 
